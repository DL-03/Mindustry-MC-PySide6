...
from PyQt6.QtCore import QPoint, QRect, QDir, QAbstractTableModel, QTime, QTimer
...
def MainL():
	...
	def SelLCpod(index = None, Type = None):
		if index != None:
			print(tree.sender().model().filePath(index))
			...
			if TempMod != None:
				if "type" in TempMod[0]:
					...
					EntryObj(TempMod[0], [EditRoot[1], TempMod[0]["type"]], tree.sender().model().filePath(index), TempMod[1])
				else:
					EntryObj(TempMod[0], [EditRoot[1], ""], tree.sender().model().filePath(index), TempMod[1])
		if Type != None:
			if Type == "mod":
	...
	def SelLC(index = None, event = None, Type = None):
		...
		try:
			...
			if RootMod[1] != "":
				if index != None:
					if os.path.isfile(tree.sender().model().filePath(index)):
						_yes = True
				else:
					_yes = True
				if _yes:
